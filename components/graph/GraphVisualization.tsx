...
import { Box, Loader, Center, Text, useMantineColorScheme, useMantineTheme } from '@mantine/core';
...
export function GraphVisualization({
  ...
  const themeAwareConfig = useMemo(() => {
    const isDark = colorScheme === 'dark';
    return {
      ...defaultConfig,
      colorScheme: {
        ...defaultConfig.colorScheme,
        background: isDark ? mantineTheme.colors.dark[8] : '#ffffff',
        edges: isDark ? '#868e96' : '#495057',
      },
      ...userConfig,
    };
  }, [colorScheme, mantineTheme, userConfig]);

  // Merge user config with defaults
  const config = themeAwareConfig;
  ...
  useEffect(() => {
    if (graphRef.current && data) {
      ...
      // Add collision force
      graph.d3Force('collision', forceCollide(config.forceSettings.collisionRadius));
    }
  }, [data, config.forceSettings]);

  // Zoom to fit only on initial data load
  useEffect(() => {
